...
pub(in crate::command) struct SandboxConfigKindArgs {
...
#[derive(Debug, clap::Args)]
#[group(required = true, multiple = false)]
pub(in crate::command) struct SandboxRunKindArgs {
    /// Run the foton tests.
    #[clap(long)]
    test: bool,
...
pub(in crate::command) enum SandboxCommand {
...
pub(in crate::command) fn dispatch(command: &SandboxCommand) -> eyre::Result<()> {
    match command {
    ...
    loop {
        ...
        let res = match watcher_rx.recv_timeout(timeout) {
            ...
            Err(RecvTimeoutError::Timeout) => {
                bail!("timed out waiting for run completion");
            }
            Err(RecvTimeoutError::Disconnected) => {
                bail!("file watcher disconnected while waiting for run completion")
            }
        };
