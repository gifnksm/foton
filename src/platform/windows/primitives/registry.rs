use std::{borrow::Cow, ffi::OsString, fmt::Display};

use snafu::{IntoError as _, OptionExt as _, ResultExt as _, Snafu};
use windows::Win32::Foundation::ERROR_FILE_NOT_FOUND;
use windows_core::HSTRING;
use windows_registry::{CURRENT_USER, Value};

use crate::{package::PackageId, util::path::AbsolutePath};

cfg_select! {
    all(test, not(build_for_sandbox)) => {
        // Registry-touching tests must run under the sandbox harness so they cannot
        // affect the user's real registry or session state.
        fn assert_sandbox_test_only() {
            if std::env::var_os("FOTON_UNSAFE_ALLOW_REGISTRY_TESTS").is_some() {
                return;
            }
            panic!("registry primitives must only run in sandbox tests; use `cargo xtask sandbox run --test` instead.");
        }
    }
    _ => {
        fn assert_sandbox_test_only() {}
    }
}

const USER_FONTS_REGISTRY_BASE_KEY: &str = r"Software\Microsoft\Windows NT\CurrentVersion\Fonts";

fn err_is_not_found(err: &windows_result::Error) -> bool {
    err.code() == ERROR_FILE_NOT_FOUND.to_hresult()
}

#[derive(Debug, Snafu)]
pub(crate) enum RegistryError {
    #[snafu(display("failed to open registry key: {path}"))]
    OpenRegistryKey {
        path: String,
        source: windows_core::Error,
    },
    #[snafu(display("failed to create registry key: {path}"))]
    CreateRegistryKey {
        path: String,
        source: windows_core::Error,
    },
    #[snafu(display("failed to remove registry key: {path}"))]
    RemoveRegistryKey {
        path: String,
        source: windows_core::Error,
    },
    #[snafu(display("failed to enumerate values of registry key: {path}"))]
    EnumerateValues {
        path: String,
        source: windows_core::Error,
    },
    #[snafu(display("failed to set registry value for font `{file_name}`: {path}"))]
    SetValue {
        file_name: String,
        path: String,
        source: windows_core::Error,
    },
    #[snafu(display("registry key for package version already exists: {path}"))]
    PackageKeyAlreadyExists { path: String },
    #[snafu(display("invalid font entry found in registry key: {path}"))]
    InvalidEntryFound {
        path: String,
        #[snafu(source(from(RegisteredFontError, Box::new)))]
        source: Box<RegisteredFontError>,
    },
}

#[derive(Debug)]
struct RegistryKey {
    base: Cow<'static, str>,
}

impl Default for RegistryKey {
    fn default() -> Self {
        Self::with_base(USER_FONTS_REGISTRY_BASE_KEY)
    }
}

impl RegistryKey {
    fn with_base<P>(base: P) -> Self
    where
        P: Into<Cow<'static, str>>,
    {
        assert_sandbox_test_only();
        Self { base: base.into() }
    }

    fn path(&self, app_id: &str, pkg_id: &PackageId) -> String {
        fn assert_registry_path_segment<S>(kind: &str, segment: S)
        where
            S: Display,
        {
            let segment = segment.to_string();
            assert!(
                !segment.contains(['\\', '\0', '@']) && !segment.chars().any(char::is_control),
                "invalid registry path segment for {kind}: {segment:?}"
            );
        }

        assert_registry_path_segment("app id", app_id);
        assert_registry_path_segment("package name", pkg_id.name());
        assert_registry_path_segment("package version", pkg_id.version());
        let base = &self.base;
        let name = pkg_id.name();
        let version = pkg_id.version();
        format!(r"{base}\{app_id}@{name}@{version}")
    }

    fn list_package_fonts(
        &self,
        app_id: &str,
        pkg_id: &PackageId,
    ) -> Result<Vec<RegisteredFont>, RegistryError> {
        let path = self.path(app_id, pkg_id);

        let key = match CURRENT_USER.open(&path) {
            Ok(key) => key,
            Err(err) if err_is_not_found(&err) => {
                return Ok(vec![]);
            }
            Err(source) => {
                return Err(OpenRegistryKeySnafu { path }.into_error(source));
            }
        };

        key.values()
            .context(EnumerateValuesSnafu { path: &path })?
            .map(|(name, value)| {
                RegisteredFont::from_reg(name, value)
                    .context(InvalidEntryFoundSnafu { path: &path })
            })
            .collect()
    }

    fn register_package_fonts<I, F>(
        &self,
        app_id: &str,
        pkg_id: &PackageId,
        fonts: I,
    ) -> Result<(), RegistryError>
    where
        I: IntoIterator<Item = F>,
        F: AsRef<RegisteredFont>,
    {
        let path = self.path(app_id, pkg_id);

        match CURRENT_USER.open(&path) {
            Ok(_) => return Err(PackageKeyAlreadyExistsSnafu { path }.build()),
            Err(err) if err_is_not_found(&err) => {}
            Err(source) => return Err(OpenRegistryKeySnafu { path }.into_error(source)),
        }

        let key = CURRENT_USER
            .create(&path)
            .context(CreateRegistryKeySnafu { path: &path })?;

        for font in fonts {
            let font = font.as_ref();
            key.set_value(font.reg_name(), &font.reg_value())
                .context(SetValueSnafu {
                    file_name: &font.file_name,
                    path: &path,
                })?;
        }

        Ok(())
    }

    fn unregister_package_fonts(
        &self,
        app_id: &str,
        pkg_id: &PackageId,
    ) -> Result<(), RegistryError> {
        let path = self.path(app_id, pkg_id);

        if let Err(source) = CURRENT_USER.remove_tree(&path)
            && !err_is_not_found(&source)
        {
            return Err(RemoveRegistryKeySnafu { path }.into_error(source));
        }

        Ok(())
    }
}

pub(crate) fn list_registered_package_fonts(
    app_id: &str,
    pkg_id: &PackageId,
) -> Result<Vec<RegisteredFont>, RegistryError> {
    RegistryKey::default().list_package_fonts(app_id, pkg_id)
}

pub(crate) fn register_package_fonts<I, F>(
    app_id: &str,
    pkg_id: &PackageId,
    fonts: I,
) -> Result<(), RegistryError>
where
    I: IntoIterator<Item = F>,
    F: AsRef<RegisteredFont>,
{
    RegistryKey::default().register_package_fonts(app_id, pkg_id, fonts)
}

pub(crate) fn unregister_package_fonts(
    app_id: &str,
    pkg_id: &PackageId,
) -> Result<(), RegistryError> {
    RegistryKey::default().unregister_package_fonts(app_id, pkg_id)
}

#[derive(Debug, Snafu)]
pub(crate) enum RegisteredFontError {
    #[snafu(display("registered font file name is not valid UTF-8: {file_name}", file_name = file_name.display()))]
    FileNameNotUtf8 { file_name: OsString },
    #[snafu(display("registered font path for `{reg_name}` has invalid value type"))]
    InvalidFontPathValueType {
        reg_name: String,
        source: windows_core::Error,
    },
    #[snafu(display("registered font path for `{file_name}` is not an absolute path: {path}", path = path.display()))]
    FontPathIsNotAbsolute { file_name: String, path: OsString },
}

#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord)]
pub(crate) struct RegisteredFont {
    file_name: String,
    path: AbsolutePath,
}

impl AsRef<RegisteredFont> for RegisteredFont {
    fn as_ref(&self) -> &RegisteredFont {
        self
    }
}

impl RegisteredFont {
    pub(crate) fn new<T, P>(file_name: T, path: P) -> Result<Self, RegisteredFontError>
    where
        T: Into<OsString>,
        P: Into<AbsolutePath>,
    {
        let file_name = file_name.into();
        let path = path.into();
        let file_name = file_name
            .into_string()
            .map_err(|title| FileNameNotUtf8Snafu { file_name: title }.build())?;
        Ok(Self { file_name, path })
    }

    pub(crate) fn path(&self) -> &AbsolutePath {
        &self.path
    }

    fn from_reg(reg_name: String, reg_value: Value) -> Result<Self, RegisteredFontError> {
        let path = HSTRING::try_from(reg_value)
            .context(InvalidFontPathValueTypeSnafu {
                reg_name: &reg_name,
            })?
            .to_os_string();
        let path = AbsolutePath::new(&path).context(FontPathIsNotAbsoluteSnafu {
            file_name: &reg_name,
            path: &path,
        })?;
        Ok(Self {
            file_name: reg_name,
            path,
        })
    }

    fn reg_name(&self) -> &str {
        &self.file_name
    }

    fn reg_value(&self) -> Value {
        Value::from(&HSTRING::from(self.path.as_path()))
    }
}

#[cfg(test)]
mod tests {
    use std::{
        process,
        str::FromStr as _,
        sync::atomic::{AtomicUsize, Ordering},
    };

    use crate::APP_ID;

    use super::*;

    struct RegistryTestGuard {
        reg_key: RegistryKey,
        app_id: &'static str,
    }

    impl RegistryTestGuard {
        fn new() -> Self {
            static TEST_ID: AtomicUsize = AtomicUsize::new(0);
            let test_id = TEST_ID.fetch_add(1, Ordering::Relaxed);
            let pid = process::id();
            let base = format!(r"{USER_FONTS_REGISTRY_BASE_KEY}\foton-test-{pid}-{test_id}");
            let reg_key = RegistryKey::with_base(base);
            let app_id = APP_ID;
            cleanup_registry(&reg_key);
            Self { reg_key, app_id }
        }
    }

    impl Drop for RegistryTestGuard {
        fn drop(&mut self) {
            cleanup_registry(&self.reg_key);
        }
    }

    fn with_registry_test<T>(f: impl FnOnce(&RegistryKey, &str) -> T) -> T {
        let guard = RegistryTestGuard::new();
        f(&guard.reg_key, guard.app_id)
    }

    fn test_package_id(name: &str) -> PackageId {
        format!("registry-test-{name}@0.1.0").parse().unwrap()
    }

    fn cleanup_registry(reg_key: &RegistryKey) {
        let base = &reg_key.base;
        assert_ne!(base, USER_FONTS_REGISTRY_BASE_KEY);
        if let Err(err) = CURRENT_USER.remove_tree(base)
            && !err_is_not_found(&err)
        {
            panic!("failed to remove registry key `{base}`: {err:?}");
        }
    }

    #[test]
    #[cfg_attr(
        not(build_for_sandbox),
        ignore = "registry should be isolated in sandbox tests. use `cargo xtask sandbox run --test` instead."
    )]
    fn list_package_fonts_returns_empty_for_missing_package() {
        with_registry_test(|reg_key, app_id| {
            let pkg_id = test_package_id("missing-list");

            let entries = reg_key.list_package_fonts(app_id, &pkg_id).unwrap();

            assert!(entries.is_empty());
        });
    }

    #[test]
    #[cfg_attr(
        not(build_for_sandbox),
        ignore = "registry should be isolated in sandbox tests. use `cargo xtask sandbox run --test` instead."
    )]
    fn unregister_package_fonts_ignores_missing_package() {
        with_registry_test(|reg_key, app_id| {
            let pkg_id = test_package_id("missing-unregister");

            reg_key.unregister_package_fonts(app_id, &pkg_id).unwrap();
        });
    }

    #[test]
    #[cfg_attr(
        not(build_for_sandbox),
        ignore = "registry should be isolated in sandbox tests. use `cargo xtask sandbox run --test` instead."
    )]
    fn register_list_and_unregister_package_fonts_roundtrip() {
        with_registry_test(|reg_key, app_id| {
            let pkg_id = test_package_id("roundtrip");

            let expected_entries = [
                RegisteredFont::new(
                    "example-font-a.ttf",
                    AbsolutePath::new(r"C:\path\to\example-font-a.ttf").unwrap(),
                )
                .unwrap(),
                RegisteredFont::new(
                    "example-font-b.ttf",
                    AbsolutePath::new(r"C:\path\to\example-font-b.ttc").unwrap(),
                )
                .unwrap(),
            ];

            reg_key
                .register_package_fonts(app_id, &pkg_id, &expected_entries)
                .unwrap();

            let mut actual_entries = reg_key.list_package_fonts(app_id, &pkg_id).unwrap();
            actual_entries.sort_by(|lhs, rhs| lhs.reg_name().cmp(rhs.reg_name()));

            let mut expected_entries = expected_entries;
            expected_entries.sort_by(|lhs, rhs| lhs.reg_name().cmp(rhs.reg_name()));

            assert_eq!(actual_entries.len(), expected_entries.len());
            for (actual, expected) in actual_entries.iter().zip(expected_entries.iter()) {
                assert_eq!(actual.reg_name(), expected.reg_name());
                assert_eq!(actual.path(), expected.path());
            }

            reg_key.unregister_package_fonts(app_id, &pkg_id).unwrap();

            let entries_after_unregister = reg_key.list_package_fonts(app_id, &pkg_id).unwrap();
            assert!(entries_after_unregister.is_empty());
        });
    }

    #[test]
    #[cfg_attr(
        not(build_for_sandbox),
        ignore = "registry should be isolated in sandbox tests. use `cargo xtask sandbox run --test` instead."
    )]
    fn list_package_fonts_errors_on_non_string_value() {
        with_registry_test(|reg_key, app_id| {
            let pkg_id = test_package_id("invalid-value");

            let path = reg_key.path(app_id, &pkg_id);
            let key = CURRENT_USER.create(&path).unwrap();
            key.set_u32("Invalid Font", 42).unwrap();

            let err = reg_key.list_package_fonts(app_id, &pkg_id).unwrap_err();
            match err {
                RegistryError::InvalidEntryFound {
                    path: err_path,
                    source,
                } => {
                    assert_eq!(err_path, path);
                    match *source {
                        RegisteredFontError::InvalidFontPathValueType {
                            reg_name: name, ..
                        } => {
                            assert_eq!(name, "Invalid Font");
                        }
                        other @ (RegisteredFontError::FileNameNotUtf8 { .. }
                        | RegisteredFontError::FontPathIsNotAbsolute { .. }) => {
                            panic!("unexpected registered font error: {other:?}")
                        }
                    }
                }
                other => panic!("unexpected registry error: {other:?}"),
            }
        });
    }

    #[test]
    #[cfg_attr(
        not(build_for_sandbox),
        ignore = "registry should be isolated in sandbox tests. use `cargo xtask sandbox run --test` instead."
    )]
    fn unregister_package_fonts_removes_package_key() {
        with_registry_test(|reg_key, app_id| {
            let pkg_id = test_package_id("cleanup-package-key");

            reg_key
                .register_package_fonts(
                    app_id,
                    &pkg_id,
                    [RegisteredFont::new(
                        "example-font.ttf",
                        AbsolutePath::new(r"C:\path\to\example-font.ttf").unwrap(),
                    )
                    .unwrap()],
                )
                .unwrap();

            let path = reg_key.path(app_id, &pkg_id);
            CURRENT_USER.open(&path).unwrap();

            reg_key.unregister_package_fonts(app_id, &pkg_id).unwrap();

            let err = CURRENT_USER.open(&path).unwrap_err();
            assert!(err_is_not_found(&err));
        });
    }

    #[test]
    #[cfg_attr(
        not(build_for_sandbox),
        ignore = "registry should be isolated in sandbox tests. use `cargo xtask sandbox run --test` instead."
    )]
    fn unregister_package_fonts_keeps_other_package_keys() {
        with_registry_test(|reg_key, app_id| {
            let pid = process::id();
            let pkg_id_v1 =
                PackageId::from_str(&format!("registry-test-cleanup-keep-keys@0.1.0-pid-{pid}"))
                    .unwrap();
            let pkg_id_v2 = PackageId::from_str(&format!(
                "registry-test-cleanup-keep-keys-other@0.2.0-pid-{pid}",
            ))
            .unwrap();

            let entries = [RegisteredFont::new(
                "example-font.ttf",
                AbsolutePath::new(r"C:\path\to\example-font.ttf").unwrap(),
            )
            .unwrap()];

            reg_key
                .register_package_fonts(app_id, &pkg_id_v1, &entries)
                .unwrap();
            reg_key
                .register_package_fonts(app_id, &pkg_id_v2, &entries)
                .unwrap();

            reg_key
                .unregister_package_fonts(app_id, &pkg_id_v1)
                .unwrap();

            let path_v1 = reg_key.path(app_id, &pkg_id_v1);
            let path_v2 = reg_key.path(app_id, &pkg_id_v2);
            let err = CURRENT_USER.open(&path_v1).unwrap_err();
            assert!(err_is_not_found(&err));
            CURRENT_USER.open(&path_v2).unwrap();
        });
    }

    #[test]
    #[cfg_attr(
        not(build_for_sandbox),
        ignore = "registry should be isolated in sandbox tests. use `cargo xtask sandbox run --test` instead."
    )]
    fn register_package_fonts_errors_when_package_version_already_exists() {
        with_registry_test(|reg_key, app_id| {
            let pkg_id = test_package_id("duplicate-register");
            let entries = [RegisteredFont::new(
                "example-font.ttf",
                AbsolutePath::new(r"C:\path\to\example-font.ttf").unwrap(),
            )
            .unwrap()];

            let path = reg_key.path(app_id, &pkg_id);
            let key = CURRENT_USER.create(&path).unwrap();
            key.set_value(entries[0].reg_name(), &entries[0].reg_value())
                .unwrap();

            let err = reg_key
                .register_package_fonts(app_id, &pkg_id, &entries)
                .unwrap_err();
            match err {
                RegistryError::PackageKeyAlreadyExists { path: err_path } => {
                    assert_eq!(err_path, path);
                }
                other => panic!("unexpected registry error: {other:?}"),
            }
        });
    }
}
